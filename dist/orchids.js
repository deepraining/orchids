...
if(false) {
	...
	if(!content.locals) {
		module.hot.accept("!!../../node_modules/.0.28.7@css-loader/index.js!./orchids.css", function() {
			var newContent = require("!!../../node_modules/.0.28.7@css-loader/index.js!./orchids.css");
...
var util = __webpack_require__(0),
    ...
    fragment = __webpack_require__(12),
...
app.init = function (option) {
    ...
    if (option && option.parentContainer) {
        var domContainer,
            type = _typeof(option.parentContainer);
        // selector
        if (type == 'string') domContainer = document.getElementById(option.parentContainer);
        // dom
        else if (type == 'object' && option.parentContainer.nodeType == 1 && typeof option.parentContainer.nodeName == 'string') domContainer = option.parentContainer;else console.error('orchids: unknown parent container, it should be one of follows: id selector, dom object.');

        domContainer && (container.parentContainer = domContainer, document.body.classList.add('orchids-custom-container'));
    }
...
app.onpopstate = function (event) {
    var dialogInstance, prevDialogInstance, pageInstance, i, il, dialogsInstancesKeys;
    ...
    if (!!dialogsInstancesKeys.length) {
        for (il = dialogsInstancesKeys.length, i = il - 1; i >= 0; i--) {
            // first dialog
            i <= 0 ? (pageInstance = app.getCurrentPage(), dialogInstance = app.dialogsInstances[dialogsInstancesKeys[i]], dialogInstance.forResult && !!pageInstance.page.onPageResult && pageInstance.page.onPageResult(dialogInstance.dialog.__orchids__result || {}),
            // destroy
            dialogInstance.dialog.__orchids__destroy(), app.deleteCurrentDialog()) : (
            // at least two dialogs
            prevDialogInstance = app.dialogsInstances[dialogsInstancesKeys[i - 1]], dialogInstance = app.dialogsInstances[dialogsInstancesKeys[i]], dialogInstance.forResult && !!prevDialogInstance.dialog.onDialogResult && prevDialogInstance.dialog.onDialogResult(dialogInstance.dialog.__orchids__result || {}),
            // destroy
            dialogInstance.dialog.__orchids__destroy(), app.deleteCurrentDialog());
        }
    }
...
app.startPageInner = function (pageName, data, forResult, prepareResultData) {
    var pageObject = app.pages[pageName],
        // the Page Object
    option,
        // Page option
    instance,
    ...
    pageObject.option.singleton && (app.pagesSingletonInstances[pageName] = {
        id: option.pageId,
        page: instance
    });
...
app.startDialogInner = function (dialogName, data, forResult, prepareResultData) {
    var dialogObject = app.dialogs[dialogName],
        // the Dialog Object
    option,
        // Dialog option
    instance,
    ...
    dialogObject.option.singleton && (app.dialogsSingletonInstances[dialogName] = {
        id: option.dialogId,
        dialog: instance
    });
...
app.registerPage = function (pageName, extendAttributes, option, superPageName) {
    var newPage,
        // new Page Object
    superPagesExtendAttributes = [],
        // all super extend attributes
    superPagesOptions = [],
        // all super options
    tempOption,
        i,
...
app.registerFragment = function (fragmentName, extendAttributes, option, superFragmentName) {
    var newFragment,
        // new Fragment Object
    superFragmentsExtendAttributes = [],
        // all super extend attributes
    superFragmentsOptions = [],
        // all super options
    tempOption,
        i,
        il;
...
app.back = function () {
    ...
    app.option.route ? Object.keys(app.pagesInstances).length >= 2 && history.back() : app.pageBack();
};
...
app.pageBack = function () {
    ...
    // for result
    instance.forResult && !!prevInstance.page.onPageResult && prevInstance.page.onPageResult(instance.page.__orchids__result || {});
    // destroy or hide
...
app.dialogBack = function () {
    ...
    // for result
    instance.forResult && (!!prevInstance ? (!!prevInstance.dialog.onDialogResult && prevInstance.dialog.onDialogResult(instance.dialog.__orchids__result || {}), prevInstanceIsDialog = !0) : (!!prevPageInstance && !!prevPageInstance.page.onPageResult && prevPageInstance.page.onPageResult(instance.dialog.__orchids__result || {}), prevInstanceIsDialog = !1));
    // destroy or hide
...
var directionClasses = __webpack_require__(2);
...
var newPage = function newPage() {
    ...
    Page.prototype = {
        ...
        __orchids__init: function __orchids__init() {
            ...
            // extra style
            self.option.style && (styleKeys = Object.keys(self.option.style)).length && styleKeys.map(function (key) {
        ...
        __orchids__getParentContainer: function __orchids__getParentContainer() {
            var self = this;
            var type = _typeof(self.option.parentContainer);
            ...
            if (self.option.parentContainer) {
                ...
                // dom
                else if (type == 'object' && self.option.parentContainer.nodeType == 1 && typeof self.option.parentContainer.nodeName == 'string') self.parentContainer = self.option.parentContainer;else {
        ...
        __orchids__renderFragments: function __orchids__renderFragments() {
            ...
            self.option.fragmentAnimateDirection == 'vertical' ? (self.__orchids__fragmentsContainerEl.classList.add('orchids-vertical'), self.__orchids__fragmentsContainerEl.style.height = self.option.fragments.length * self.__orchids__fragmentHeight + 'px') : (self.__orchids__fragmentsContainerEl.classList.add('orchids-horizontal'), self.__orchids__fragmentsContainerEl.style.width = self.option.fragments.length * self.__orchids__fragmentWidth + 'px');
        ...
        showFragment: function showFragment(id) {
            ...
            !instance.__orchids__initialized ? (!!instance.onCreate && instance.onCreate(), instance.__orchids__initialized = !0) : !!instance.onShow && instance.onShow();
            ...
            self.option.fragmentAnimateDirection == 'vertical' ? self.__orchids__fragmentsContainerEl.style.transform = 'translateY(' + (0 - self.__orchids__fragmentHeight * (id - 1)) + 'px)' : self.__orchids__fragmentsContainerEl.style.transform = 'translateX(' + (0 - self.__orchids__fragmentWidth * (id - 1)) + 'px)';
        },
        /**
         * get fragment specified by id, default return the first fragment
         * @param id
...
var newFragment = function newFragment() {
    ...
    Fragment.prototype = {
        ...
        __orchids__init: function __orchids__init() {
            ...
            self.option.fragmentDirection == 'vertical' ? (self.el.classList.add('orchids-vertical'), self.el.style.top = self.option.fragmentHeight * (self.id - 1) + 'px', self.el.style.height = self.option.fragmentHeight + 'px') : (self.el.classList.add('orchids-horizontal'), self.el.style.left = self.option.fragmentWidth * (self.id - 1) + 'px', self.el.style.width = self.option.fragmentWidth + 'px');
            // user custom initialization
            self.__orchids__isFirstFragment && (self.__orchids__initialized = !0, !!self.onCreate && self.onCreate(),
            // render fragments
        ...
        __orchids__renderSubFragments: function __orchids__renderSubFragments() {
            ...
            self.option.subFragmentAnimateDirection == 'vertical' ? (self.__orchids__subFragmentsContainerEl.classList.add('orchids-vertical'), self.__orchids__subFragmentsContainerEl.style.height = self.option.subFragments.length * self.__orchids__subFragmentHeight + 'px') : (self.__orchids__subFragmentsContainerEl.classList.add('orchids-horizontal'), self.__orchids__subFragmentsContainerEl.style.width = self.option.subFragments.length * self.__orchids__subFragmentWidth + 'px');
        ...
        showSubFragment: function showSubFragment(id) {
            ...
            // create sub fragments if not created
            !!instance.option.subFragments && !!instance.option.subFragments.length && instance.__orchids__renderSubFragments();
            self.option.subFragmentAnimateDirection == 'vertical' ? self.__orchids__subFragmentsContainerEl.style.transform = 'translateY(' + (0 - self.__orchids__subFragmentHeight * (id - 1)) + 'px)' : self.__orchids__subFragmentsContainerEl.style.transform = 'translateX(' + (0 - self.__orchids__subFragmentWidth * (id - 1)) + 'px)';
        },
